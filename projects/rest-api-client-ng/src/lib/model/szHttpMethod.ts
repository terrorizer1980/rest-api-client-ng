...
/**
 * The HTTP method that was used for the operation.  The possible values are\"   * `GET` - An HTTP GET operation.   * `POST` - An HTTP POST operation.   * `PUT` - An HTTP PUT operation.   * `PATCH` - An HTTP PATCH operation.   * `DELETE` - An HTTP DELETE operation.
 */
export type SzHttpMethod = 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE';

export const SzHttpMethod = {
    GET: 'GET' as SzHttpMethod,
    POST: 'POST' as SzHttpMethod,
    PUT: 'PUT' as SzHttpMethod,
    PATCH: 'PATCH' as SzHttpMethod,
    DELETE: 'DELETE' as SzHttpMethod
};